...
提取规则：
- 姓名从文本开头提取；如果用户同时用中英文描述，优先提取中文信息
- 联系方式识别邮箱格式和手机号格式
...
分类标准：
- must_have：明确写出的硬性要求（工作年限、特定技能、学历门槛），不要从措辞推测
...
7. 如果你在任务描述中看到"用户偏好"段落，优先遵循用户的侧重点和调整方向

如果你在任务描述中看到"系统提醒"段落，说明上游解析环节出现了问题。
请根据具体提醒内容调整策略：
- "未识别到姓名" → 简历中个人信息部分留空或标注"待补充"，不要编造
...
2. 每段经历一个 ## 标题，严格按照以下格式输出：

## 项目：{{项目名称}}
- 时间：{{时间段，格式 YYYY.MM-YYYY.MM}}
- 角色：{{职位名称}}
- 技术栈：{{用逗号分隔的技术列表}}
- 详情：
   {{每个成就或职责用独立段落，以动词开头，用数字量化成果}}
